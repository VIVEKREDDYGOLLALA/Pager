...
def estimate_text_to_fit(hindi_text, bbox_width_inches, bbox_height_inches, box_id, output_json_path, x1, y1, font_size, bboxes):
    output_folder_path = 'output_jsons2'  # Change this to your desired output folder path
    ...
    max_lines = 1  # Limit to one line per bbox
...
def generate_latex(image_path, image_dimensions, bboxes, hindi_texts, label_mapping):
    ...
    padding_points = 5
    label_counter = {'ordered list': 1, 'unordered list': None, 'option': 'A'}
    ...
    for bbox in bboxes:
        ...
        # headline_labels = [
        #     "headline", "header", "subhead", "section title", "sub section title", 
        ...
        # tikz_text_style = (
        #     "hinditext" if label.lower() in [hl.lower() for hl in headline_labels] else "paragraphtext"
        # )
        ...
        #         if label.lower().startswith("paragraph") or label.lower() == "answer"or label.lower() == "table caption":
        ...
        #                     box_id, json_file_path, x1, y1, font_size, bboxes
        #                 )
        #                 if hindi_text_to_fit:
        #                     font_size_command = font_size
        #                     break

        #         # Fallback to "Large" font size if no text fits
        #         if not hindi_text_to_fit:
        #             font_size_command = "\\large"
        #             hindi_text_to_fit = estimate_text_to_fit(
        #                 ' '.join(hindi_texts), bbox_width_inches, bbox_height_inches,
        #                 box_id, json_file_path, x1, y1, font_size_command, bboxes
        #             )

        #             # If it still doesn't fit, set a placeholder
        #             if not hindi_text_to_fit:
        #                 hindi_text_to_fit = " "  # Final fallback placeholder
...
def set_uniform_font_size_by_label(bboxes, font_size_mapping):
    """
    Assign a uniform font size to bounding boxes grouped by their label.
    ...
    Returns:
        dict: A mapping of labels to their assigned font size commands.
    """
    label_groups = defaultdict(list)

    # Group bounding boxes by their label
    for label, coords, bbox_id, _ in bboxes:
        label_groups[label].append(coords[3])  # Store heights for each label group

    font_size_by_label = {}

    # Process each group to find minimum height and assign font size
    for label, heights in label_groups.items():
        min_height = min(heights)
        closest_font_size = find_closest_font_size(min_height, font_size_mapping)
        font_size_by_label[label] = closest_font_size

    return font_size_by_label
...
label_mapping = {
    ...
    "footnote": {"font_size": "\\veryHuge", "style": ""},
...
for bbox_file_path, image_path in bbox_files_and_paths:
    ...
    font_size_by_label = set_uniform_font_size_by_label(bboxes, font_size_mapping)

    # Update label_mapping with font sizes for each label
    for label, font_size in font_size_by_label.items():
        if label in label_mapping:
            label_mapping[label]["font_size"] = font_size

    # Pass the updated label_mapping to the next function
