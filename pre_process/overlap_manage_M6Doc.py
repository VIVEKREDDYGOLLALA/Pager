import os
import re
pattern = re.compile(r'\[([a-zA-Z0-9_]+), \[([0-9.]+), ([0-9.]+), ([0-9.]+), ([0-9.]+)\], ([a-zA-Z0-9_]+), (\d+)\]')
...
def process_bboxes(file_path):
    ...
    try:
        ...
        # Remove boxes with height less than 15
        bbox_details = [box for box in bbox_details if box[4] >= 15]

        # Write the updated bounding boxes to the file
